...
const BAKAL = {
  ...
  campaigns: {
    'daf-idf': {
      ...
      status: 'active',        // active | prep | paused | done
      channel: 'email',        // email | linkedin | multi
      ...
      sequence: [
        {
          id: 'E1', type: 'email', label: 'Email initial', timing: 'J+0',
          ...
          stats: { open: 68, reply: 4.2, stop: 0.4 }
        },
        {
          id: 'E2', type: 'email', label: 'Email valeur', timing: 'J+3',
          subType: 'Case study',
          ...
          stats: { open: 72, reply: 3.1, stop: 0.8 }
        },
        {
          id: 'E3', type: 'email', label: 'Email relance', timing: 'J+7',
          ...
          stats: { open: 55, reply: 1.4, stop: 0 }
        },
        {
          id: 'E4', type: 'email', label: 'Email break-up', timing: 'J+12',
          subType: 'Soft close',
          ...
          stats: { open: 48, reply: 0.5, stop: 0 }
        }
      ],
      ...
      info: {
        period: '27 jan. → En cours (28 jours)',
        copyDesc: 'Pro décontracté · Vous · Court (3 phrases) · CTA question ouverte · FR'
      }
    },
    ...
    'dirigeants-formation': {
      ...
      sequence: [
        {
          id: 'L1', type: 'linkedin', label: 'Note de connexion', timing: 'J+0',
        ...
        {
          id: 'L2', type: 'linkedin', label: 'Message post-connexion', timing: 'J+3',
          ...
          stats: { reply: 6.8, interested: 3, stop: 1.2 }
        }
      ],
      ...
      info: {
        period: '3 fév. → En cours (20 jours)',
        copyDesc: 'Pro décontracté · Vous · Court · CTA question ouverte · FR'
      }
    },
    ...
    'drh-lyon': {
      ...
      sequence: [
        ...
        {
          id: 'L1', type: 'linkedin', label: 'Note de connexion LinkedIn', timing: 'J+1',
        ...
        {
          id: 'E2', type: 'email', label: 'Email valeur', timing: 'J+4',
          subType: 'Case study',
        ...
        {
          id: 'L2', type: 'linkedin', label: 'Message LinkedIn', timing: 'J+5',
        ...
        {
          id: 'E3', type: 'email', label: 'Email relance', timing: 'J+8',
          subType: 'Angle différent',
          subject: null,
        ...
        {
          id: 'E4', type: 'email', label: 'Email break-up', timing: 'J+13',
          subType: 'Soft close',
          subject: null,
      ...
      info: {
        period: '18 février 2026',
        createdDate: '18 février 2026',
        volumeDesc: '187 prospects · ~100/semaine',
        copyDesc: 'Formel & Corporate · Vous · CTA offre directe · FR',
        channelsDesc: 'Email (4) + LinkedIn (2) · 13 jours',
        launchEstimate: '~25 février (après validation)'
      }
    }
  },
  ...
  reports: [
    {
      week: 'Semaine 4 — Rapport consolidé',
      dateRange: '10 — 16 février 2026',
      score: 'excellent',
      scoreLabel: '🚀 Excellent',
      metrics: { contacts: 247, openRate: '62%', replyRate: '8.1%', interested: 5, meetings: 3 },
      synthesis: "<strong>Performance globale en hausse.</strong> La campagne \"DAF Île-de-France\" est votre meilleure performeuse cette semaine avec 9.2% de taux de réponse (+2.1pts vs S3). L'angle \"douleur client\" continue de surperformer sur le segment comptabilité. La campagne LinkedIn \"Dirigeants Formation\" progresse mais reste sous les objectifs de réponse (6.8% vs 8% cible).<br><br><strong>Canaux :</strong> L'email reste le canal le plus efficace (62% d'ouverture). LinkedIn montre un potentiel de conversion supérieur (ratio réponse→RDV de 33% vs 22% par email).<br><br><strong>Prochaine action recommandée :</strong> Lancer une campagne hybride Email+LinkedIn sur le segment \"Dirigeants PME 11-50\" qui combine le reach email et la conversion LinkedIn."
    },
    {
      week: 'Semaine 3',
      dateRange: '3 — 9 février 2026',
      score: 'good',
      scoreLabel: '🟢 Performant',
      metrics: { contacts: 220, openRate: '58%', replyRate: '6.8%', interested: 3, meetings: 2 },
      synthesis: "Bonne progression du taux d'ouverture grâce à l'optimisation des objets d'email (A/B test gagné : \"Question rapide sur [secteur]\" +8pts vs ancien objet). Le segment comptabilité confirme son potentiel. Recommandation appliquée S2 (changement d'angle) porte ses fruits."
    },
    {
      week: 'Semaine 2',
      dateRange: '27 jan. — 2 fév. 2026',
      score: 'ok',
      scoreLabel: '🟡 Correct',
      metrics: { contacts: 185, openRate: '51%', replyRate: '5.2%', interested: 2, meetings: 1 },
      synthesis: "Démarrage solide. Les taux sont dans la moyenne du marché B2B. L'angle \"preuve sociale\" fonctionne moins bien que prévu sur le segment Formation. Recommandation : tester l'angle \"douleur client\" sur la prochaine séquence."
    }
  ],

  /* ═══ PERFORMANCE CHART DATA (4 weeks) ═══ */
  chartData: [
...
function renderCampaignsTable() {
  ...
  return campaigns.map(c => {
    ...
    if (isPrep) {
      ...
      const replyColor = c.kpis.replyRate >= 8 ? 'var(--success)' : 'var(--warning)';
      ...
      const replyColor = c.kpis.replyRate >= 8 ? 'perf-good' : 'perf-ok';
      const replyPct = Math.min(c.kpis.replyRate * 10, 100);
      replyHtml = `<div style="font-weight:600;">${c.kpis.replyRate}%</div><div class="perf-bar"><div class="perf-fill ${replyColor}" style="width:${replyPct}%"></div></div>`;
      meetingsHtml = `<span style="font-weight:700;color:var(--success)">${c.kpis.meetings}</span>`;
    }
...
function initFromData() {
  // Overview KPIs
  document.querySelector('#section-overview .kpi-grid').innerHTML = renderGlobalKpis();
  ...
  // Opportunities
  const oppsContainer = document.querySelector('#section-overview .section-grid .card:nth-child(3) .card-body > div');
  if (oppsContainer) oppsContainer.innerHTML = renderOpportunities();

  // AI Recommendations
  const recosContainer = document.querySelector('#section-overview .section-grid .card:nth-child(4) .card-body > div');
